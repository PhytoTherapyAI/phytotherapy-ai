...
import { PRICING, formatTry } from "@/lib/pricing"

// Plan label table is BUILT from lib/pricing.ts — never hardcode the
// TRY amount here. If a price changes the source of truth lives in
// lib/pricing.ts and this table is regenerated by the helper below.
type PlanKey =
...
const PLAN_LABELS: Record<PlanKey, { tr: { name: string; price: string }; en: { name: string; price: string } }> = (() => {
  ...
  const indSavings = `%${ind.savingsPercent} tasarruf`
  const indSavingsEn = `${ind.savingsPercent}% savings`
  const famSavings = `%${fam.savingsPercent} tasarruf`
  const famSavingsEn = `${fam.savingsPercent}% savings`
  return {
    "individual-monthly": {
      tr: { name: "Premium Bireysel — Aylık", price: `${formatTry(ind.monthly)} / ay` },
      en: { name: "Premium Individual — Monthly", price: `${formatTry(ind.monthly)} / mo` },
    },
    "individual-yearly": {
      tr: { name: `Premium Bireysel — Yıllık (${indSavings})`, price: `${formatTry(ind.yearly)} / yıl · ≈ ${formatTry(ind.yearlyMonthlyEquivalent)} / ay` },
      en: { name: `Premium Individual — Yearly (${indSavingsEn})`, price: `${formatTry(ind.yearly)} / yr · ≈ ${formatTry(ind.yearlyMonthlyEquivalent)} / mo` },
    },
    "family-monthly": {
      tr: { name: "Premium Aile — Aylık", price: `${formatTry(fam.monthly)} / ay · ${fam.maxMembers} kişiye kadar` },
      en: { name: "Family Premium — Monthly", price: `${formatTry(fam.monthly)} / mo · up to ${fam.maxMembers} people` },
    },
    "family-yearly": {
      tr: { name: `Premium Aile — Yıllık (${famSavings})`, price: `${formatTry(fam.yearly)} / yıl · ≈ ${formatTry(fam.yearlyMonthlyEquivalent)} / ay · ${fam.maxMembers} kişiye kadar` },
      en: { name: `Family Premium — Yearly (${famSavingsEn})`, price: `${formatTry(fam.yearly)} / yr · ≈ ${formatTry(fam.yearlyMonthlyEquivalent)} / mo · up to ${fam.maxMembers} people` },
    },
  }
})()
